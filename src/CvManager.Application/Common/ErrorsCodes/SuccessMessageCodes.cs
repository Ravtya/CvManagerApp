namespace CvManager.Application.Common.ErrorsCodes;

public static class SuccessMessageCodes
{
    public const string ProfileSaved = "Success.ProfileSaved";
    public const string RecordCreated = "Success.RecordCreated";
    public const string RecordUpdated = "Success.RecordUpdated";
    public const string PasswordChanged = "Success.PasswordChanged";
    public const string PasswordSet = "Success.PasswordSet";
    public const string CvSaved = "Success.CvSaved";
    public const string CvCreated = "Success.CvCreated";
    public const string EmailConfirmed = "Success.EmailConfirmed";
    public const string ConfirmationSent = "Success.ConfirmationSent";
    public const string SalesforceExportSuccess = "Salesforce.ExportSuccess";
    public const string SupportTicketCreated = "Success.SupportTicketCreated";
}
